...
class EMA:
    ...
    def update_model_avg(self, ma_model, current_model):
        for current_params, ma_params in zip(current_model.parameters(), ma_model.parameters()):
            old_weight, up_weight = ma_params.data, current_params.data
            ma_params.data = self.update_avg(old_weight, up_weight)
    ...
    def step_ema(self, ema_model, model, step_start_ema = 2000):
        if self.step < step_start_ema:
            ...
            self.step += 1
            return
        self.update_model_avg(ema_model, model)
        self.step += 1
...
if __name__ == "__main__":
    model = UNet(c_in = 1536, c_out=1536, time_dim=28, device='cpu')
    print(model)